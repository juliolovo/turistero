...
7. La API usa el rol de la aplicación (`DATABASE_URL`, solo filas). El rol migrador (`DATABASE_MIGRATOR_URL`, solo migraciones) es para `db:migrate`/CI y el administrador (`DATABASE_ADMIN_URL`) solo para tareas manuales de roles: nunca los pongas en Vercel ni en el código de la API.
...
npm run db:migrate     # aplica migraciones con el rol MIGRADOR (DATABASE_MIGRATOR_URL); sin URL usa PGlite
npm run db:up          # Postgres local en Docker (turistero_db_dev; roles turistero_admin_usr, turistero_migrator_usr y turistero_app_usr)
