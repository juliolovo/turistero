...
# Crea el rol administrador, el migrador (solo migraciones) y el rol de la aplicación (solo DML). Contraseñas: variables de entorno del contenedor.
set -eu
...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" \
  ...
  -v migrator_role="$MIGRATOR_DB_USER" -v migrator_pwd="$MIGRATOR_DB_PASSWORD" \
  -f /sql/01-admin-role.sql -f /sql/02-app-role.sql -f /sql/03-migrator-role.sql
...
echo "✓ Roles creados: administrador=$ADMIN_DB_USER, migrador=$MIGRATOR_DB_USER (migraciones) y aplicación=$APP_DB_USER (solo filas) en $POSTGRES_DB"
