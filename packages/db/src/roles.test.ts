 ...
 *   - rol ADMIN: dueño del esquema, crea/altera/elimina tablas y ejecuta migraciones;
...
describe.skipIf(!adminUrl || !appUrl)("permisos de los roles de Postgres", () => {
  ...
  beforeAll(async () => {
    const m = await createDb({ url: adminUrl!, migrate: true }); // el ADMIN aplica las migraciones
    await m.close();
  ...
  it("los roles no son superusuarios ni pueden crear bases de datos o roles", async () => {
    const r = await admin`select rolname, rolsuper, rolcreatedb, rolcreaterole from pg_roles where rolname in (current_user, ${new URL(appUrl!).username})`;
