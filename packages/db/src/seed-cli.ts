...
const { db, close, kind } = await createDb({ url: process.env.DATABASE_MIGRATOR_URL ?? process.env.DATABASE_ADMIN_URL ?? process.env.DATABASE_URL, migrate: true, dataDir: process.env.PGLITE_DIR });
console.log(`DB: ${kind}`);
